...
def trace_lineage_through_subqueries(cytoscape_data, temp_tables, current_database=''):
    ...
    # 🎯 新增：补充识别被SQLLineage合并遗漏的子查询节点
    def identify_missing_subquery_nodes():
        """识别被SQLLineage合并遗漏的子查询节点"""
        missing_subqueries = set()
        
        # 查找所有字段节点，检查其parent_candidates
        for node_id, node_data in nodes_dict.items():
            if node_data.get("type") == "Column" and "." in node_id:
                parent_candidates = node_data.get("parent_candidates", [])
                ...
                for candidate in parent_candidates:
                    if isinstance(candidate, dict):
                        ...
                        # 如果parent是SubQuery类型，但该名称不在已识别的subquery_nodes中
                        if (candidate_type == "SubQuery" and 
                            candidate_name not in subquery_nodes and
                            candidate_name not in missing_subqueries):
                            missing_subqueries.add(candidate_name)
                            print(f"🔍 发现遗漏的子查询节点: {candidate_name}")
        
        return missing_subqueries
    
    # 补充遗漏的子查询节点
    missing_subqueries = identify_missing_subquery_nodes()
    subquery_nodes.update(missing_subqueries)
    ...
    def is_subquery_column(column_id):
        """判断字段是否属于SubQuery（增强版：检查parent_candidates）"""
        if not column_id or '.' not in column_id:
            return False
        
        # 方法1：直接从表名部分判断
        table_part = column_id.split('.')[0]
        if table_part in subquery_nodes:
            return True
        
        # 方法2：从字段的parent_candidates判断
    ...
    # 🎯 修复：处理所有子查询字段的血缘关系，不使用processed_subquery_columns限制
    if subquery_nodes:
        print(f"🔍 处理子查询血缘关系，共有 {len(subquery_nodes)} 个子查询节点: {subquery_nodes}")
        
        # 🔍 调试：输出所有边信息
        print(f"🔍 调试：所有边数据 ({len(edges)} 条):")
        for i, edge in enumerate(edges):
            source_id = edge.get("source", "")
            target_id = edge.get("target", "")
            print(f"  边{i+1}: {source_id} -> {target_id}")
            ...
            if (is_subquery_column(source_id) and
                ...
                for real_source_id in real_sources:
                    ...
                    if not has_intermediate_path(real_source_id, target_id):
                        ...
                        print(f"✅ 添加追踪血缘: {real_source_id} -> {target_id}")
                    else:
...
select 
c.id , c.age , d.id , d.age
from (select id , age from db2.t2) c
left join (select id , age from db2.t2) d on c.id  = d.id 
